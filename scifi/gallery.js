...
for (let i=0; i < g.length; i++) { //loop through all gallery items
...
// 'this' in this case is c[0], or the first child of the gallery div, the a element around the image.
	...
	if (img[0].getAttribute("alt") != null) { // if there's an alt text
...
function prevItem(p) {
	if (p==0) { p = g.length-1; } else { p--; } //previous index with loop around to end
	prev = g[p].children; //the gallery div representing the previous image
	overlayPrev.onclick = function() { showOverlay(prev[0],p); };
}
...
function nextItem(n) {
	if (n==g.length-1) { n = 0; } else { n++; } //next image with loop around to beginning
	next = g[n].children;
	overlayNext.onclick = function() { showOverlay(next[0],n); };
}
